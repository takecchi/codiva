...
- ignore 済みファイルの引き継ぎ `ignoredFiles`: `'symlink'`（既定・複製コストゼロ）/
  ...
  **`.codiva/` と `.git` は必ず除外**（再帰・内部状態破壊の防止）。実体化は entry 単位の
  ...
  - **列挙済みディレクトリの中身は捨てる**（`topLevelEntries()`）。除外の出所がディレクトリの
    中にあると（husky v9 の `.husky/_/` = 中に `*` の `.gitignore`）、ls-files は親と中身を両方返す。
    親をリンクした後に子を `rm` + `symlink` すると**リンク越しに元リポジトリの実体を消し、
    自己参照リンクに置き換える**（メインの husky が壊れた）。I/O 側も書く前・消す前に
    祖先がリンクでないこと（`crossesSymlink`）を確かめる。
