...
`utils/grok.ts` / `utils/title.ts` を触る前に読む。**
...
- **中断は「走っているターン」にしか効かない。始まる前に中断されたターンは投げない。**
  `Ctrl+C` はセッションの立ち上げ中にも押せる。そこで cancel を送っても空振りするので、
  そのあと素直に prompt を出すと **UI は中断済みなのにエージェントだけが動く**
  （完了イベントは中断フラグで抑止されるため、誰も気付けない）。長寿命プロセスの provider は
  「プロセスを殺せば止まる」が成り立たないので、**指示を捨てる**ことで中断を表現する。
