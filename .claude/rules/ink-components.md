...
- **スクロールする領域の高さを、その領域の状態で変えない**。ログの直下に出る行
  ...
  - **常時 1 行の枠を「増やす」前に、既存の 1 行と排他かを疑う。** `DETAIL_CHROME_ROWS` は
    `dialogMaxRows` が引き算に使うので、1 行増やすと**低い端末で許可/質問ダイアログの選択肢が
    窓に 1 件も入らなくなる**（実測: 24 行の端末。番人は `tests/app.test.tsx` の「低い端末でも
    Ask がログを潰さず…」）。サブエージェントの実行状況は独立した行にしようとしてこれを踏み、
    結局操作ヒント行と同居させた（優先順位: 認証 → 再開 → サブエージェント → `Ctrl+C` → 空行。
    `Ctrl+C` より優先してよいのは、あれがフォーカス横断の chord で案内が無くても効くのに対し
    「何が走っているか」はその行でしか分からないから）。
- **ログの機械は 1 実装に統一する（`ui/log-pane.tsx` の `useLogPane` + `<LogPane>`）。**
  会話ログを描く画面は 2 つある（セッション詳細・サブエージェント詳細）が、**仕様は 1 つ**:
  可視域の実測・末尾寄せ・スクロール（キー/ホイール）・範囲選択とコピー・URL のクリック・
  端の自動スクロール・幅変更や先頭 `seq` 変更での選択破棄。ここは「簡略コピー」が黙って壊れる
  場所（`flexShrink={0}` 忘れ → 行の虫食い、幅変更のリセット忘れ → 触っていない行がコピーされる）
  なので、新しくログを描く画面を足すときも必ずこれを使う。`useInput` はフックに持たせない
  （1 画面 1 `useInput` は view のまま）で、`handleMouse` が boolean を返して**押下の裁定順を
  view に残す**（`useComposer.handleMouse` と同じ契約）。状態行（常時 1 行）は `<LogPane>` の
  中に含める — あれは「ログの高さを状態で変えない」ための予約行なので、離すと片方の画面で
  条件付きに戻されうる。
- **読み取り専用の画面には入力欄を置かない。** サブエージェントには指示を送れないので、
  `SubagentDetail` はコンポーザを持たず出口も Esc 一本にする（`/exit` すら置かない）。
  代わりに `Ctrl+C` は**親セッションの**ターンを中断する（暴走したサブエージェントを止めたいのは
  まさにその画面なので、chord の意味を画面間で揃える）。
- **画面遷移を起こすクリックは press ではなく release で**（`drag` が来たら取り消す）、
  **左ボタン限定**、**ラベルの表示幅の内側だけ**を当たりにする（行末より右は何も無い余白なので、
  そこで遷移すると意図しない副作用になる）。許可/質問待ちの間は遷移しない —
  アンマウントすると `PermissionDialog` の内部 state（何問目か・書きかけの自由記述）が捨てられる。
