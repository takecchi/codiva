...
- **詳細ビューでもマウス捕捉は解除しない**（かつては `mouse.disable()` でネイティブ選択に
  ...
  - **スクロール位置（アンカー）は ref にも持ち、必ず ref の現在値から次を計算する**。端末は
    ホイール連打や自動スクロールの tick を同じ tick にまとめて届けるので、`setAnchor(fn)` だけだと
    全部が同じ stale な値から計算されて 1 回分に潰れる（`bufferRef` と同じ理由）。
