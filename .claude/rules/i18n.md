...
- **UI コンポーネント（`.tsx`）に表示文字列を直書きしない。** 文字列は `src/core/i18n.ts` の
  ...
  - もう一つの例外は **外部システムが唯一の出所である表示文字列**。現状これは
    `/model` のモデル名・説明文だけ（Claude Code の `supportedModels()` 由来、英語のみ）。
    翻訳するとモデルが増えるたびにカタログ更新が必要になり、避けたかった直書きの
    陳腐化がカタログ側へ移るだけになるため、SDK の文字列をそのまま出す。
    ダイアログの見出し・操作ヒント・取得中表示のような **codiva 自身の文言は
    カタログに置く**（`model.title` / `model.help` / `model.loading` / `model.saved`）。
...
1. `Messages` interface に適切なグループでキーを足す。現行グループは
   `list` / `detail` / `action`（両 view 共通の確認・エラー）/ `badge` / `permission` / `model` /
   `prompt` / `notify` / `resume` / `auth` / `banner` / `footer` / `command` の 13。
   両 view で同じ文言になるものは `action` に置く（`list.*` と `detail.*` に重複させない）。
