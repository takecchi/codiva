...
/**
 * Claude Code アダプタの「失敗の見分け方」。
 *
 * ここに集めたのは **Claude CLI の文言・typed error kind・HTTP ステータスの知識** で、
 * どれも provider が変われば意味を失う（Codex に "OAuth session expired" は無い）。
 * 状態機械（`core/status-reducer.ts` / `core/agent-events.ts`）はこの知識を持たず、
 * 分類結果の {@link AgentStopCause} だけを受け取る。別のエージェントを足すときは、
 * このファイルの対になるものをそのアダプタ用に書く。
 *
 * `USAGE_LIMIT_ERROR_PREFIXES` を SDK から読んでいるのは意図的: 使用制限の文言は
 * CLI 側で変わるので、**Claude アダプタの中でだけ** SDK に追従させるのが正しい
 * （中立モジュールは SDK を import しない、が守るべき境界）。
 */
 ...
 *
 * 旧 `core/status-reducer.ts` から移設。状態機械が特定エージェントの SDK 定数を
 * import している状態を解消するのが目的で、判定内容は変えていない。
 */
...
/**
 * 構造を持たない失敗（query が throw した / `errors[]` の 1 行）を
 * {@link AgentStopCause} へ分類する。`AgentAdapter.classifyError` の Claude 実装。
 *
 * **順序に意味がある**:
 *  1. 認証切れ — 待っても再試行しても直らないので最優先。レート制限の文言と
 *     紛らわしいケース（"request timed out" を含む認証エラー）で誤分類しないため。
 *  2. レート制限 — 待てば直る。
 *  3. 通信断 — 再開すれば続きから走る。
 *  4. それ以外は本物の失敗。
 */
export function classifyClaudeError(text: string): AgentStopCause {
  if (isAuthError(text)) {
    return 'auth';
  }
