 ...
 * （認証 / 再開 / サブエージェントの実行状況 / Ctrl+C / 空行。同じく常に 1 行）+
 * 入力欄 3（上下ボーダー付き）+ フッタ 1。
 *
 ...
 *
 * **行を増やすときはここも必ず増やす。** ただし増やす前に「既存の 1 行と排他か」を
 * 疑うこと — サブエージェントの実行状況は独立した行にしようとして 9 にしたが、
 * `dialogMaxRows` がここから引き算するため 24 行の端末で**質問ダイアログの選択肢が
 * 1 件も窓に入らなくなった**（番人は `tests/app.test.tsx` の「低い端末でも Ask が
 * ログを潰さず…」）。結局あれは操作ヒント行と排他（走っている間は Ctrl+C の案内より
 * 優先、終わっていれば認証・再開の案内が優先）だったので同じ 1 行に畳んである。
 */
