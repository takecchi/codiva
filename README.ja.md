...
> 対象の Git リポジトリで起動し、指示ごとに独立した git worktree 上でコーディングエージェント（Claude Code / Codex / Grok / Antigravity）のセッションを並列実行する TUI アプリ。
...
**このページは日本語版です。English documentation is available in [README.md](./README.md).**
...
**エージェントは Claude Code 専用ではありません。** 次の 4 つから選べます（一覧の `/agent` で切り替え。詳しくは「[エージェントを選ぶ・切り替える](#エージェントを選ぶ切り替えるagent)」）。
...
| **Grok** | xAI の `grok` CLI |
| **Antigravity** | Google の [`agy` CLI](https://antigravity.google/docs/cli/) |
...
- **エージェントを選べる** — Claude Code / **Codex**（`codex` CLI）/ **Grok**（xAI の `grok` CLI）/ **Antigravity**（Google の `agy` CLI）に対応。一覧の `/agent` で新規セッションの既定を選び（自動で保存）、詳細ビューの `/agent` でそのセッションを途中から切り替えられる。導入・ログイン状態も `/agent` に表示され、`/login` で codiva の中からサインインできる（Antigravity だけは `agy` に `login` サブコマンドが無いので対象外）。
...
- **プラン / 使用状況の表示**（Claude のとき） — 一覧画面のヘッダに claude.ai のプラン種別（Pro / Max / Team / Enterprise）と使用リミット枠（使用率・リセットまでの残り時間）を表示。既定エージェントが Codex / Grok / Antigravity のときは、報告されないので表示しません。
...
- Node.js **>= 20**
- **`claude` / `codex` / `grok` / `agy`（Antigravity）の CLI が 1 つ以上**インストール・ログイン済みであること（下記「エージェントを選ぶ・切り替える」）。
  どれも無くても codiva は起動でき、`/login` で codiva の中からサインインできます（`agy` は `agy` を素で起動してサインインしてください）。
...
### ツール実行のログをまとめる（`Ctrl+O` / `/tools`）

エージェントの作業ログは `⏺ Bash(…)` / `⎿ …` の 2 行組が延々と続くので、放っておくと**会話の本文（説明と自分の指示）がツールの実況に埋もれます**。codiva は**連続したツール実行を 1 行のまとめへ畳んで**表示します。
...
- **開くのはクリック 1 回**（まとめ行のどこでも構いません）。開いた行は `▾` になり、中身がその下に出ます。もう一度クリックすると畳みます。
- **`Ctrl+O` は全部まとめて開閉**します（Claude Code と同じキー）。コマンドパレットの **`/tools`** でも同じことができます。許可待ちのダイアログが出ている間も効きます。
- **いま動いている作業は畳みません**。ログのいちばん後ろのまとまりはそのまま全部見えていて、次の本文や指示が届いた時点で畳まれます。「何をしているか分からない」状態にはなりません。
- **ツール呼び出しが 1 件だけのまとまりも畳みません**。`Read src/core/scroll.ts` の 1 行はそれ自体で読めるので、畳むとファイル名が消えて情報が減るだけだからです。
- 畳んでもログが失われるわけではありません（表示だけの操作です）。まとめの内訳は種類ごとの件数（読み込み / 編集 / コマンド実行 / 検索 …）です。
- 既定を変えたいときは `~/.codiva/config.json` の `collapseToolLogs` を `false` にします（`/config` からも切り替えられます）。畳まない設定でも `Ctrl+O` でいつでもまとめられます。

### 自分の発言を見つけやすくする

会話ログの**自分の指示（`>` の行）には地の色**が付きます。長いやり取りを遡るとき、文字を読まなくても「どこで自分が喋ったか」が分かるようにするためです。

### サブエージェントの実行状況を追う（`/subagents`）

エージェントが**サブエージェント**（Claude Code の Task ツール。調査や下請け作業を別のエージェントに任せる仕組み）を起こすと、詳細ビューのログ下段に 1 行で状況が出ます。

```
⏺ サブエージェント: general-purpose · 実行中 · Writing report.txt · Write · 12s
```

- 出るのは**種別・状態・今やっていること・直近に使ったツール・経過時間**です。「今やっていること」はサブエージェント側の進捗に合わせて書き換わります。
- **その行をクリックすると、そのサブエージェント専用のログ画面**に入ります（`Esc` で元の詳細ビューへ戻ります）。マウスを使わない場合は **`/subagents`** と入力してください。
- 複数走っているときは 1 行に「他 N 件」と畳まれ、クリック（または `/subagents`）で一覧から選べます。
- **サブエージェントが内部で実行したツールは、親のログには混ざりません**（専用ログ側にまとまります）。親のログには「何を任せたか」の 1 行だけが残ります。
- 終わったあとも記録は残るので、後から結果や作業内容を読み返せます（セッション 8 本ぶんまで。codiva を再起動すると消えます）。
- 専用ログ画面に**入力欄はありません** — サブエージェントに直接指示は送れないためです。その画面の `Ctrl+C` は親セッションのターンの中断になります。
- 専用ログ画面でも**ツール実行のまとめ**（`Ctrl+O` / クリック）・範囲選択・URL のクリックは詳細ビューと同じように使えます（ログの実装が 1 つなので挙動が食い違いません）。
- サブエージェントを報告するのは今のところ **Claude Code だけ**です（Codex / Grok / Antigravity では何も出ません）。
...
一覧画面のヘッダ（バナー）に、ログイン中の claude.ai プランと使用リミット枠が出ます（Codex / Grok / Antigravity はこれらを報告しないため、既定をそちらにしている間は出ません）。
...
- **ヘッダは「次に動くエージェント」の説明**です。`/agent` で既定を Codex / Grok / Antigravity に
  切り替えると、エージェント名・プラン・モデル・使用状況が揃って切り替わります（いずれもプランや
  使用状況を報告しないので、その間はプラン行とゲージが消え、モデルは `モデル: CLI 既定` に戻ります）。
...
セッションを動かすエージェントは **Claude Code**（`claude` CLI / Claude Agent SDK）・**Codex**（OpenAI の `codex` CLI）・**Grok**（xAI の `grok` CLI）・**Antigravity**（Google の `agy` CLI）から選べます。既定は Claude ですが、Claude が入っていない環境でも他の 3 つだけで使えます。

1. 使いたい CLI をインストールしてログインを済ませる（`claude` → `claude auth login` / `codex` → `codex login` / `grok` → `grok login` / `agy` → **`agy` を引数なしで起動してサインイン**）。Grok は `curl -fsSL https://x.ai/cli/install.sh | bash`、Antigravity は `curl -fsSL https://antigravity.google/cli/install.sh | bash` で入ります（`agy` は `~/.local/bin` に入るので、PATH を通してください。ドキュメントは <https://antigravity.google/docs/cli/>）。**codiva はこれらの CLI を同梱しません** — `git` や `gh` と同じく、あなたの環境に入っているコマンドを起動します（使わない人にまで大きなバイナリを配らないため）。認証も codiva は一切触りません。
2. **一覧で `/agent` と入力**し、使いたいエージェントを選ぶだけで新規セッションの既定になります（設定ファイルの手編集は不要 — 選ぶと `~/.codiva/config.json` に保存されます。もちろん `"agent": "codex"` / `"agent": "grok"` / `"agent": "antigravity"` を直接書いても構いません）。ダイアログには各エージェントの**導入・ログイン状態**（`使用できます` / `未ログイン` / `未導入`）が並びます。
3. すでに動いているセッションを途中で切り替えるなら、詳細ビュー（一覧で `Enter`）で **`/agent`** を選びます。切替は**次の指示から**効きます。

`claude` も `codex` も `grok` も `agy` も入っていない状態でも codiva は起動します。その場合は一覧に「コーディングエージェントが見つかりません」と出るので、案内どおりどれかを入れてログインしてください。
...
**Antigravity だけは codiva の中からサインインできません。** `agy` には `login` サブコマンドが無く、**引数なしで起動したフル TUI の中でしかサインインできない**ためです（それを codiva の中で動かすには端末を明け渡すしかなく、他のセッションの表示が壊れます）。別のターミナルで `agy` を起動してサインインを済ませてください。`/agent` のダイアログには導入・ログイン状態だけが出て、ログインの導線は出ません。
...
worktree の分離・並列実行・追加指示・中断（`Ctrl+C`）・マージ / 破棄・PR 自動化（`/sync` / `/fix-ci`）・デスクトップ通知・リポジトリ追加指示（`.codiva/prompt.md`）は**どのエージェントでも同じように使えます**。違うのは次の 6 点だけです。

| | Claude Code | Codex | Grok | Antigravity |
|---|---|---|---|---|
| ツール使用の許可 / 質問ダイアログ | ✅ | ❌（サンドボックスで代替） | ✅ | ❌（実行モードで代替） |
| コスト表示（ヘッダの合計金額） | ✅ | ❌ | ❌ | ❌ |
| プラン / 使用状況ゲージ | ✅ | ❌ | ❌ | ❌ |
| 再起動後のログ復元 | ✅ | ❌（会話の再開自体は可） | ❌（同） | ❌（同） |
| `/model` によるモデル選択 | ✅ | ✅ | ✅ | ❌（設定の `model` を起動時に渡すだけ） |
| codiva の中からのサインイン（`/login`） | ✅ | ✅ | ✅ | ❌（`agy` を素で起動する） |
...
**Antigravity セッションの制約**（Claude セッションとの違い）:

- **ツール使用の許可を尋ねません。** headless の `agy` は許可の判断を CLI の内部で行い、個々のツール実行を外部へ中継する仕組みを持ちません。Codex と同じ理由（そして同じ結論）で、codiva は「それらしい許可ダイアログ」を出さず、**実行モードを安全弁**にしています。`質問あり` の状態にもなりません。
  - 既定は `--mode=accept-edits`（codiva の既定の許可モード `acceptEdits` に対応）です。`permissionMode` を `"plan"` にすると `--mode=plan`（計画だけを立てて書き換えない）になります。
  - **全許可（`--dangerously-skip-permissions`）は、`permissionMode` に `"bypassPermissions"` を明示したときだけ**渡します。「許可を尋ねられない」ことと「全部を無条件に通す」ことは別の話なので、既定にはしていません。
- **`/model` が出ません。** `agy models` は人間向けのテキストしか出さず（JSON 出力のフラグが無い）、`--model` は**起動時のフラグ**で走っているセッションのモデルを変えられないためです。使いたいモデルは `~/.codiva/config.json` の `model` に書いてください（セッション起動時に `--model` として渡ります）。選択肢を出せないのにダイアログを出すと、空か他のエージェントのモデル名が並ぶことになるので出していません。なお**実際に動いているモデル名はセッション一覧に出ます**（`agy` が起動時に報告するため）。
- **コストを表示しません。** `agy` はターンの終わりにトークン数しか返さず、金額もアカウント全体の使用状況も運びません。ヘッダの合計コストには Antigravity のセッションを数えず、プラン表示と使用状況ゲージ（Claude のアカウントの話）も既定を Antigravity にしている間は出ません。
- **codiva の中からサインインできません**（上記）。`agy` を別のターミナルで起動してサインインしてください。ログイン状態の判定には `agy models` を使っていて、「サインインしてください」と言われたときだけ `未ログイン` と表示します。オフラインなどで判定できなかったときは**未ログイン扱いにしません**（使えるはずのエージェントに誤った案内を出さないため）。
- **再起動後にログが復元されません**（セッションの続きを再開すること自体はできます）。ログの再構築は Claude CLI の記録ファイルを読む仕組みで、Antigravity の記録は形式が異なるためです。
- **サブエージェントの実行状況は出ません。** `agy` の出力にはそれらしい項目がありますが、開始と終了の対応を実データで確認できていないためです。中途半端に追跡すると「終わったのに終わったと分からない」セッションを作りかねないので、確認できるまで出しません。
- 中断（`Ctrl+C`）は `agy` のプロセスごと止めますが、**会話はそのまま続けられます**（次の指示で同じ会話を再開します）。同じ理由で、キャンセルされたターンや終了イベントを出さずに落ちたターンは「失敗」ではなく「中断」として扱われ、そのまま追加指示を送れます。
### ツール実行の許可モード（`shift+tab`）

画面下部のフッタに今のモードが出ます。`shift+tab` で切り替わります。

| モード | 表示 | 挙動 |
|---|---|---|
| 自動 | `⏵⏵ 自動モード` | 質問以外のツールをすべて自動実行（既定） |
| 確認 | `⏸ 確認モード` | すべてのツールで許可ダイアログを出す |
| リスク判定 | `⏵? リスク判定モード` | **オプトイン**。ツールごとにリスクを判定し、ルーチンなものだけ自動実行する |

「質問」（`AskUserQuestion` 相当）は**どのモードでも必ず**ダイアログに出ます。それ自体が「ユーザーに聞く」経路なので、自動許可すると空の回答で承諾を返してしまうためです。

#### リスク判定モード（Jev / TypeSafe AI）

`自動モード` は速いかわりに `git push --force` も黙って走り、`確認モード` は安全なかわりに `ls` 1 回ごとに手が止まります。その中間として、**ツール実行ごとにリスクを判定して「ルーチンなものだけ自動実行し、判断がつかないものは確認ダイアログへ上げる」**モードを用意しています。判定には [Jev](https://docs.typesafe.ai)（TypeSafe AI の判断専用モデル。文章を書かず、型付きの問いに確率つきで答える）を使います。

**既定では完全に無効**で、有効にしない限り `shift+tab` の輪にも現れません（従来どおり 自動 ⇄ 確認 の 2 モードのままです）。

有効化には**両方**が必要です。

1. `~/.codiva/config.json` に `"jev": { "enabled": true }` を書く
2. 環境変数 `TYPESAFE_API_KEY` を設定する（キーは [console.typesafe.ai](https://console.typesafe.ai/settings/keys) で発行。**設定ファイルには書けません**）
...
両方が揃うと起動時のモードが `リスク判定モード` になります（`shift+tab` でいつでも 確認 / 自動 へ切り替えられます）。

**判定は最終決定になりません。** codiva が自動実行するのは Jev が「ルーチン」と答え、かつその確率が `allowThreshold`（既定 0.9）以上のときだけです。それ以外は**すべて**確認ダイアログへ上がります。

| Jev の答え | codiva の動き |
|---|---|
| `allow`（確率 ≥ しきい値） | 自動実行 |
| `allow`（確率 < しきい値） | 確認ダイアログ |
| `ask` | 確認ダイアログ |
| `deny` | **自動拒否はせず**確認ダイアログ |
| タイムアウト / API エラー / オフライン / キー切れ | 確認ダイアログ |

つまり **Jev が壊れても「勝手に走る」側には倒れません**。落ちている間は実質 `確認モード` と同じになります。API が 3 回続けて失敗したら 60 秒は問い合わせ自体を止めるので、オフラインやキー切れのまま使っても**ツールごとに `timeoutMs` 待たされることはありません**。

> [!IMPORTANT]
> **外部 API へ何が送られるか。** リスク判定モードで走っているセッションが許可を求めるたびに、TypeSafe AI の API（`https://api.typesafe.ai/v1/systemone`）へ次の情報が送られます。
>
> - ツール名（`Bash` など）と中立化したツール種別（`shell` / `edit` / `read` …）
> - **絞り込んだ**ツール入力 — シェルコマンド・ファイルパスなど。ただし
>   - ファイル本文・差分（`content` / `old_string` / `new_string` / `patch` など）は**中身を送らず**「`<5000 chars>`」のような大きさの目印に置き換えます
>   - キー名が鍵・トークンらしいもの（`token` / `secret` / `password` / `apiKey` …）は**キーごと落とします**
>   - 文字列は 400 文字、フィールド数は 12 件で打ち切ります
> - 直近のユーザー指示（600 文字まで）と、動かしているエージェント名（`claude` / `codex` / `grok` / `antigravity`）
>
> リポジトリの内容そのもの（ファイルの中身・diff・git 履歴）は送りません。とはいえ**コマンドラインやファイルパスは送られる**ので、それが許されない環境では有効にしないでください。無効（既定）の場合、codiva は TypeSafe AI へ一切通信しません。

送信先を自前のゲートウェイ等へ向けたいときは `jev.baseUrl`（または環境変数 `TYPESAFE_BASE_URL`）を使います。

なお**ツール使用の許可を上げられないエージェント（Codex / Antigravity）では機能しません**。フッタは `⏵? リスク判定 (非対応)` と出ます。
...
- `agent`: 新しいセッションを既定でどのエージェントで動かすか。`"claude"`（既定）/ `"codex"` / `"grok"` / `"antigravity"`。**一覧の `/agent` で選ぶとここへ自動保存**されるので、通常は手で書く必要はありません。セッションごとの途中切替は詳細ビューの `/agent` です（上記）。
...
- `jev`: リスク判定モード（上記）の設定。既定は無効で、`enabled: true` **かつ**環境変数 `TYPESAFE_API_KEY` があるときだけ有効になります。**API キーはここに書けません**（設定ファイルに平文で置かせないため、環境変数からのみ読みます）。
  - `enabled`: 有効化。既定 `false`。
  - `timeoutMs`: 判定 1 回の上限（ms）。既定 `1500`。超えたら確認ダイアログへ倒します。
  - `allowThreshold`: 自動実行に踏み切る確率のしきい値（0〜1）。既定 `0.9`。上げるほど確認ダイアログが増えます。
  - `model`: モデル id。既定 `"jev-latest"`。
  - `baseUrl`: API のベース URL。既定 `"https://api.typesafe.ai"`（環境変数 `TYPESAFE_BASE_URL` でも指定できます）。
- `collapseToolLogs`: セッション詳細の会話ログで、連続したツール実行を 1 行のまとめに畳むか。既定 `true`。`false` にすると従来どおり全行を出します（どちらの設定でも `Ctrl+O` / `/tools` でその場で切り替えられます）。
...
利用できるスラッシュコマンドは、コンポーザで `/` を入力するとパレット表示されます（`/prompt`・`/config`・`/model`・`/agent`（一覧=既定の選択 / 詳細=そのセッションの切替）・`/login`（codiva 内でサインイン）・`/subagents`（詳細ビュー）・`/sync`・`/fix-ci`・`/recover`・`/remove`・`/clear`・`/update`・`/help` など）。端末が低くて一覧が入り切らないときは末尾が「他 N 件」に畳まれるので、続けて文字を打って絞り込んでください（全件は `/help`）。
...
> ドキュメントは日本語（このファイル）を原本とし、英語版 [README.md](./README.md) はその訳です。
> 利用者可視の挙動を変えたときは**両方**を更新してください（`docs/` と `.claude/` は現状日本語のみ）。
