...
- サブエージェントを報告するのは今のところ **Claude Code だけ**です（Codex / Grok では何も出ません）。
...
> - 直近のユーザー指示（600 文字まで）と、動かしているエージェント名（`claude` / `codex` / `grok`）
>
...
なお**ツール使用の許可を上げられないエージェント（Codex）では機能しません**。フッタは `⏵? リスク判定 (非対応)` と出ます。
