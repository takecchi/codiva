...
- **エージェントの切替** — セッションを動かすエージェントとして Claude Code のほかに **Codex**（`codex` CLI）を選べる。既定は設定 `agent`、セッションごとの切替は詳細ビューの `/agent`（下記）。
...
2. 既定を切り替えるなら `~/.codiva/config.json` に `"agent": "codex"` を書きます（以降の**新規セッション**が Codex になります）。
3. セッション単位で切り替えるなら、詳細ビュー（一覧で `Enter`）の入力欄で **`/agent`** と入力して選びます。切替は**次の指示から**効きます。
...
- `agent`: 新しいセッションを既定でどのエージェントで動かすか。`"claude"`（既定）/ `"codex"`。セッションごとの切替は詳細ビューの `/agent` です（上記）。
...
利用できるスラッシュコマンドは、コンポーザで `/` を入力するとパレット表示されます（`/prompt`・`/model`・`/agent`（詳細ビュー）・`/sync`・`/fix-ci`・`/recover`・`/remove`・`/clear`・`/update`・`/help` など）。
