...
- 質問が出ているセッションを選んでいても、`Tab` をもう一度押せば一覧ゾーンへ抜けられるので、**`↑↓` で別のセッションへ切り替えられます**。一覧ゾーンではダイアログは表示されたまま（内容は読めます）ですが、キーは受け取りません。回答に戻りたいときは `Tab` をもう一周（入力欄 → ダイアログ）させるか、`Enter` で詳細ビューを開いてそこで答えます。
- 回答の途中（何問目か・チェック済みの選択肢・書きかけの自由記述）は `Tab` で往復しても保持されます。
...
利用できるスラッシュコマンドは、コンポーザで `/` を入力するとパレット表示されます（`/prompt`・`/model`・`/sync`・`/fix-ci`・`/recover`・`/remove`・`/clear`・`/update`・`/help` など）。
