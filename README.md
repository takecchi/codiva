...
> - your most recent instruction (up to 600 characters) and which agent is driving (`claude` / `codex` / `grok` / `antigravity`)
>
...
Note that it **does nothing on agents that can't surface tool permissions (Codex / Antigravity)**; the footer reads `⏵? smart mode (n/a)` there.
