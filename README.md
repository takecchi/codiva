...
セッション詳細のログは**直近 2000 件**までを保持し、それより古い行は落とします（1 件が極端に
長い場合も途中で切り、末尾に `…` を付けます）。長時間動かしたセッションのログを無制限に抱えると
codiva 自身がメモリ不足で落ちるためで、会話の完全な記録は Claude Code 側のトランスクリプト
（`~/.claude/projects/`）に残っています。
