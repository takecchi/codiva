...
- （任意）Codex を使う場合は `codex` CLI がインストール済みで `codex login` 済みであること（下記）
...
利用できるスラッシュコマンドは、コンポーザで `/` を入力するとパレット表示されます（`/prompt`・`/model`・`/agent`（一覧=既定の選択 / 詳細=そのセッションの切替）・`/sync`・`/fix-ci`・`/recover`・`/remove`・`/clear`・`/update`・`/help` など）。
