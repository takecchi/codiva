...
対象Gitリポジトリで起動し、指示ごとに独立した git worktree 上でコーディングエージェント（Claude Code = Claude Agent SDK / Codex CLI / Grok CLI / Antigravity CLI = `agy`）のセッションを並列実行する TUI アプリ。TypeScript (ESM, strict) + Ink 7 + npm。
...
| 別のエージェントに対応させる | `core/agent-ports.ts`（`AgentAdapter` / `AgentCapabilities` / `PermissionDecision` = DI 境界）/ `core/agent-events.ts`（`AgentEvent` の語彙 + 全 provider 共通の畳み込み `applyAgentEvent`）/ `core/claude-adapter.ts`・`core/claude-parse.ts`・`core/claude-errors.ts`（Claude 実装の 3 点セット）/ `core/codex-adapter.ts`・`core/codex-parse.ts`・`core/codex-errors.ts` + `core/codex-events.ts`（JSONL の型）・`core/codex-models.ts`・`core/codex-rollout.ts`（rollout から解決済みモデル）・`utils/codex.ts`（`codex exec` の起動 = 唯一の I/O）/ `core/grok-adapter.ts`・`core/grok-parse.ts`・`core/grok-errors.ts` + `core/grok-events.ts`（ACP メッセージの型）・`core/grok-models.ts`・`utils/grok.ts`（`grok agent stdio` の起動 = 唯一の I/O）/ `core/antigravity-adapter.ts`・`core/antigravity-parse.ts`・`core/antigravity-errors.ts` + `core/antigravity-events.ts`（stream-json の型）・`utils/antigravity.ts`（`agy --input-format=stream-json` の起動 = 唯一の I/O）/ 行区切り JSON の枠切りは provider 非依存の `core/jsonl.ts`（Codex / Grok / Antigravity 共用）/ アダプタの登録は `bootstrap/build-manager.ts` の `buildAgents` |
| SDK メッセージの解釈 | `core/claude-parse.ts` **のみ**（`parseClaudeMessage`: SDKMessage → `AgentEvent[]`）+ `core/__fixtures__/*.jsonl`。Codex は `core/codex-parse.ts`（`parseCodexEvent`: `codex exec --json` の JSONL → `AgentEvent[]`）+ `core/__fixtures__/codex-*.jsonl`。Grok は `core/grok-parse.ts`（`createGrokParser`: ACP = JSON-RPC over stdio の通知 → `AgentEvent[]`）+ `core/__fixtures__/grok-*.jsonl`。Antigravity は `core/antigravity-parse.ts`（`createAntigravityParser`: `agy` の stream-json = `init` / `step_update` / `result` の 3 種 → `AgentEvent[]`。step の ACTIVE 連打 → DONE を畳むので**状態を持つ**）+ `core/__fixtures__/antigravity-*.jsonl`（**実採取は `antigravity-autherror.jsonl` だけ**。詳細は docs/TECH_NOTES.md） |
...
| ツール実行の許可（`auto` / `smart` / `confirm`）| `core/run-mode.ts`（`RunMode` / `createModePolicy` = 同期の 3 値判定 / `nextRunMode` = shift+tab の輪。**評価器が無ければ `smart` は輪に入らない**）/ `core/session.ts` の `PermissionPolicy`・`isQuestion`・`requestPermission`（`'evaluate'` のときだけ非同期へ降りる）/ `core/permission-evaluator.ts`（`PermissionEvaluator` = DI 境界・`toPermissionContext` / `redactToolInput` = 送信データの絞り込み・`evaluatePermission` = **安全側への丸めはここ 1 か所**）/ `utils/jev.ts`（Jev = TypeSafe AI への HTTP 1 往復。**SDK パッケージは入れない**）/ 配線は `bootstrap/build-manager.ts`（設定 `jev.enabled` + 環境変数 `TYPESAFE_API_KEY` の両方が要る）/ 表示は `ui/status-footer.tsx` |
...
| サブエージェントの実行状況 | `core/subagents.ts`（記録のリスト操作・純粋。**完了ゲートとは別物**）/ `core/subagent-row.ts`（下段 1 行の文言・当たり判定・ピッカーの選択肢・純粋）/ `ui/subagent-detail.tsx`（専用ログ画面。**入力欄なし**）/ `ui/subagent-picker.tsx`（複数件の選択）/ `core/agent-events.ts` の `routeLogEntry`（内部ログ行の振り分け） |
...
| ログ描画・スクロール | `ui/log-pane.tsx`（`useLogPane` + `<LogPane>` = **会話ログを描く全画面の共通実装**）/ `core/scroll.ts`（`logLines` は**エントリ単位でメモ化**）/ `core/markdown.ts` / `core/ansi.ts` / `ui/log-line.tsx`（1 行の描画・kind ごとの地の色 `logBackground`） |
| ツール実行のログを畳む（`Ctrl+O` / `/tools`）| `core/log-collapse.ts`（どこが 1 まとまりか = `collapsibleRunAt`・まとめ行の文言 = `toolRunLabel`・純粋）/ `core/scroll.ts` の `logLines(…, collapse)`（まとめ見出しの行を出す。`DisplayLine.group` が取っ手）/ `core/log-selection.ts` の `logGroupAt`（当たり判定）/ `ui/log-pane.tsx`（press→release の開閉・アンカー固定）/ `ui/session-detail.tsx`（展開状態・Ctrl+O）/ 種類は `LogEntry.tool`（`AgentToolKind`）に載る |
| ログの上限・メモリ | `core/log-buffer.ts`（件数/文字数の上限・`pushLogEntry` が唯一の追記経路。予算は `LogLimits` で差し替え = サブエージェントは別枠） |
...
| モデル選択 | `core/models.ts` / `utils/model-catalog.ts`（Claude）/ `core/codex-models.ts` + `utils/codex.ts` の `fetchCodexModelCatalog`（Codex）/ `core/grok-models.ts` + `utils/grok.ts` の `fetchGrokModelCatalog`（Grok。`initialize` の `_meta.modelState` を読む）/ `ui/model-select.tsx`。エージェントごとの選択肢は `app.tsx` の `modelsByAgent` の表で両 view へ渡す（provider ごとの ternary を増やさない）。**Antigravity はカタログを持たない**（`agy models` に JSON 出力が無く、`--model` は起動時フラグでターン中に変えられない）ので `modelCatalog` / `setModel` とも false = `/model` を出さず、設定値をそのまま `--model` へ渡す |
| 一覧に出す「実際に動いているモデル」 | Claude は `system/init` が運ぶ。**Grok も自分で報告する**（`session/new` / `session/resume` の `models.currentModelId`・ターン応答の `_meta.modelId`・`_x.ai/models/update`）。**Antigravity も `init.model` で報告する**（`--model` 未指定なら空文字で来るので、空はモデル名として扱わない）。**Codex だけが JSONL に持たない**ので `core/codex-rollout.ts`（純粋な抽出）+ `utils/codex.ts` の `resolveCodexRolloutModel`（rollout の `turn_context` を読む）が要る → いずれも `AgentEvent` の `model_resolved`（status を触らない専用イベント）。理由は docs/TECH_NOTES.md |
...
3. **エージェント固有の知識はアダプタに閉じる**。`core/` の中立モジュールは
   ...
   `codex-parse.ts` / `codex-errors.ts`、Grok 側の対は `grok-adapter.ts` / `grok-parse.ts` /
   `grok-errors.ts` で、その CLI の形の知識はそこにしか置かない。例外は行区切り JSON の
   枠切り `core/jsonl.ts` = provider 固有の知識を持たない純粋な framing）。
   provider のストリームは
