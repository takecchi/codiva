...
| ログ描画・スクロール | `ui/log-pane.tsx`（`useLogPane` + `<LogPane>` = **会話ログを描く全画面の共通実装**）/ `core/scroll.ts`（`logLines` は**エントリ単位でメモ化**）/ `core/markdown.ts` / `core/ansi.ts` / `ui/log-line.tsx`（1 行の描画） |
