...
## サブエージェント表示の follow-up（実測が足りていない部分）

- [ ] **transcript からの復元**。`core/transcript.ts` は `isSidechain` 行を捨てており、CLI の
      jsonl に `parent_tool_use_id` / task id が載るかを**実測できていない**
      （`__fixtures__/transcript-restore.jsonl` にサブエージェントのターンが含まれていない）。
      skill `sdk-spike` でサブエージェント込みのセッションを走らせ、
      `~/.claude/projects/**/*.jsonl` 側に何が載るかを確かめてから決める。載っていなければ
      **`task_notification.output_file`（サブエージェントの全出力）を読む**ほうが筋が良い。
      いまは復元後 0 件（`subagents` は transient）で、専用ログを開くと「記録が残っていません」。
- [ ] **`stream_text` / `stream_reset` の帰属**。サブエージェントのストリーミングが
      `parent_tool_use_id` を運ぶか未実測。いまはプレビューを親のものとして扱っている。
- [ ] **一覧へのサブエージェント数の表示**。バッジ列は 12 セル固定・PR セルはクリック位置を
      端末幅から逆算しているので、列を足すなら `showsAgentColumn` と同じ「桁数閾値で段階的に
      落とす」形にする必要がある（別変更）。
- [ ] **詳細ビューの表示状態をビュー切替で保つ**（`detailStateRef`）。一覧は `ListViewState` を
      `App` の ref に預けているが、詳細にはそれが無いのでログのスクロール位置と書きかけの
      追加指示がビュー切替で失われる。詳細 ⇄ サブエージェントの往復が増えたぶん体感頻度が
      上がっている（既存の欠落だが、この機能で目立つようになった）。
