...
- **設定の対話的変更（`/config`）**: モデル切替・リポジトリ指示と違い、これは `SessionManager` を**通らない**（対象が「codiva 自身の設定」でセッションの状態ではないため）。項目の表と反転は純粋な `core/config-items.ts`（`CONFIG_TOGGLES` / `configToggleRows` / `toggleConfigPatch`）、表示は `ui/config-select.tsx`、今の値の保持は `app.tsx` の state、保存は合成ルートの `bootstrap/config-store.ts`（`ConfigStore`）。
  ...
  - **書き込みは 1 本ずつ直列**にし、待っている間に来た更新は最新の 1 つへ畳む（`flush()` で待てる。終了時に `main.tsx` が待つ）。並行に投げると完了順が入れ替わり、**先に始まった古いスナップショットが後から着地して新しい設定を巻き戻す**（`/config` の直後に `/model` を触ると起きる。issue #111）。メモリ上の値だけが正しくてディスクが古い状態は次回起動まで気付けないので、唯一の書き手にした意味が消える。
  - 実ファイルへの書き込みは**一時ファイル → rename**（`utils/config.ts`）。全置換の途中で落ちると壊れた JSON が残り、`loadConfig` のフォールバックで設定が丸ごと既定へ戻る。
...
- **自作 PR の状態も追う（その PR 自身に問い合わせる）**: 自作 PR の head は使い捨ての `feat/…` で、
  worktree に checkout されていないことが多い。ブランチ名でしか聞けなかった頃はそういう PR が
  `absent` として答えられ続け、**番号だけが出てグリフも `⋯`/`?` も一生付かなかった**
  （「PR の状況がいつまでも表示されない」）。いまは既知の PR（`primaryPr`）を
  `PrLookupOptions.knownPr` で渡し、ブランチ候補が全滅したら `gh pr view <URL>` で聞く。
  解決できた PR は `pr` に昇格し（reducer が `extraPrs` から畳む）、以降は普通にグリフが付く。
  既知 PR は**最後**に試す（セッションブランチに新しく出た PR を古い PR で上書きしない）。
  **番号ではなく URL で指定する**のが要点で、`PrRef` をそのまま運ぶ — PR 番号はリポジトリごとなので、
  ...
  **参照を落として次の `extraPr` が代表に繰り上がったときは、その行を「回答済み」にしない**
  （`answered` / `lastFetched` を更新せず `prLookup: 'loading'`）。今回の答えは落とした PR に
  ついてのものなので、キャッシュ扱いにすると新しい代表が陳腐化するまで（60〜180 秒）裸の番号で
  待たされる。次の tick（20 秒）で新代表を問い合わせ、それまでは `⋯` を出す。
...
- **判定は純粋（`core/pr-recovery.ts`）**。2 段に分かれているのが要点:
  ...
  **終わった PR は詰まっていない**。`mergeStatus === 'merged'` と `'closed'`（マージされずに
  クローズ）はどちらも `stuckKinds` が空を返す。閉じた PR を競合と同じに扱うと、`autoSync` が
  誰も取り込む気の無いブランチにベースを取り込んで push し、`autoFixCi` が赤いチェックの修正を
  1 ターン課金して依頼してしまう。閉じたのは人間の判断なので、codiva からは何も提案しない
  （再開したければ GitHub で reopen する → 次のポーリングで通常の状態に戻る）。
  表示も専用グリフ（`⊘`・畳んだ灰色）にして、赤い `✗`（= 直せ）と混同させない。
